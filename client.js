...
module.exports = class Client extends EventEmitter {
  ...
  constructor({name, net, startingBlock, keyPair} = {}) {
    ...
    this.uploadPrivateData = new Map();

    this.proxyKeyPair = new Map();

    this.buyersPubKey = new Map();
  ...
  postEncryptDataTransaction(dataName, proxy) {
    ...
    console.log("The initial cipher encrypted by Alice public is: " + '\n\n' + JSON.stringify(initialEncryptedData));
  ...
  postPublicKeyTransaction(dataName, dataOwner, fee=Blockchain.DEFAULT_TX_FEE) {
  ...
  postReEncryptionKeyTransaction(dataName, proxy, fee=Blockchain.DEFAULT_TX_FEE) {

    let kp = this.proxyKeyPair.get(dataName);
    let rekey = PRE.generateReEncrytionKey(kp.sk, this.buyersPubKey.get(dataName));

    console.log("The re-encryption key is: " + '\n\n' + rekey);
  ...
  generateReEncryptionKeyPair(dataName) {
    ...
    var sk_proxy = PRE.Proxy.to_hex(kp_proxy.get_private_key().to_bytes());
    var pk_proxy = PRE.Proxy.to_hex(kp_proxy.get_public_key().to_bytes());
  ...
  decrypt(dataName) {
    let kp = this.proxyKeyPair.get(dataName);
    let decryptData = PRE.decryptData(kp.sk, this.uploadPrivateData.get(dataName));
    return decryptData;
  }

  saveEncryptedData(secretData) {
    this.secretdata = secretData;
  }

  saveReceivedPublicKey(dataName, receivedPubKey) {
    this.buyersPubKey.set(dataName,receivedPubKey);
    // this.receivedPubKey = receivedPubKey;
  }
