 ...
 */
module.exports = class PREProxy extends Client {
  ...
  postReEncryptedDataTransaction(dataName, buyer, fee=Blockchain.DEFAULT_TX_FEE) {

    PRE.reEncryption(this.reEncryptionKey.get(dataName), this.encryptedData.get(dataName));
    // console.log('Proxy rekey is:' + this.rekey);
    // console.log('The encrypted data by using re-encryption key is' + '\n\n' + JSON.stringify(this.secretData));
    console.log('The encrypted data by using re-encryption key is' + '\n\n' + JSON.stringify(this.encryptedData.get(dataName)));

    // buyer.secretData = this.secretData;
    buyer.uploadPrivateData.set(dataName,this.encryptedData.get(dataName));

    // should pass the reencrypted data to the block
