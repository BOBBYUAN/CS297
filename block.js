...
module.exports = class Block {
  ...
  constructor(rewardAddr, prevBlock, target=Blockchain.POW_TARGET, coinbaseReward=Blockchain.COINBASE_AMT_ALLOWED) {
    ...
    // For proxy to use
    this.listData = prevBlock ? new Map(prevBlock.listData) : new Map();

    this.encryptedData = prevBlock ? new Map(prevBlock.encryptedData) : new Map();

    this.reEncryptionKey = prevBlock ? new Map(prevBlock.reEncryptionKey) : new Map();

    // get the list price from the previous block if available
    this.price = prevBlock ? new Map(prevBlock.price) : new Map();
  ...
  toJSON() {
    ...
    if (this.isGenesisBlock()) {
      ...
      o.listData = Array.from(this.listData.entries());
      o.encryptedData = Array.from(this.encryptedData.entries());
    } else {
      // Other blocks must specify transactions and proof details.
  ...
  addTransaction(tx, client) {
    ...
    // set the list price to the block
    this.price.set(tx.from, tx.price);

    // set the secret data and its owner to the block
    // this.secretData.set(tx.from, 11);
    this.listData.set(tx.from, tx.data);

    // if (tx.encryptedData) {
    //   console.log(tx);
    //   this.encryptedData.set(tx.from, tx.encryptedData);
    // }
  ...
  rerun(prevBlock) {
    ...
    this.listData = new Map(prevBlock.listData);
    this.encryptedData = new Map(prevBlock.encryptedData);

    this.price = new Map(prevBlock.price);
  ...
  priceOf(addr) {
    return this.price.get(addr) || 0;
  }

  secretOf(addr) {
    return this.listData.get(addr) || 0;
  }
